...
- **Programação** – Adicionado **ID** de componetes **TS** ao configurar um item para a produção.
- **Volumes Transportadoras** – Adicionado filtro de transportadora no módulo **Volumes Transportadora**.
...
- **Produção Filial** – Agora ao concluir, a lista não irá mais para o primeiro registro, mas sim, para o próximo referente ao concluído.
...
- **Programação** – Novo botão inserido para calcular quantidade de produtos não concluídos dentro de um pedido **"Calcula Ñ Conc"**.
- **Programação** – Usuário **Ricardo** agora tem permissão para imprimir relatório para o setor Tela.
...
- **Programação** – Correção na coluna que informa a quantidade não concluída na Programação. Agora para atualizar o valor, deve-se clicar em **Calcula Tempo**.
...
- **Programação** – Adicionado nova coluna para verificar a quantidade ainda não concluída do pedido dentro da Programação.

- A coluna aparecerá clicando no filtro **"Dias em Prod."**
...
- **Ajustes de desempenho** – Ajuste de consulta Programação Filial.
...
- **Programação Filial** – Ajustado etiquetas de MONTAGEM que estavam corrompidas.
...
- **Configurador de Produto** – Adicionado TS para a Programação, somente quanto TS for selecionado.
...
- **Gerador de Lotes** – Adicionado novo botão para consultar o estoque dos itens (Não montados + Estoque).

## **Versão 1.9.6.14 a 1.9.6.16**

*Data de lançamento: 27/04/2026*

Descrição da atualização do sistema.

- **Consultas gerais** – Atualizado algumas consultas gerais para melhorias.
- **Programação** – Ajuste para não derrubar o sistema caso não tenha registro de "Falta Material".
...
- **Controle de Estoque** – Removido alteração para 'EM MONTAGEM' no setor Estoque ao alterar quantidade de Estoque.
